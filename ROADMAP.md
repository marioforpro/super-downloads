...
- **R-SD-003** · download reliability (360p fix) — SHIPPED in **v1.1.1** (2026-05-31). Root cause: bundled yt-dlp was `2026.01.29` (~4mo stale) → YouTube fell back to 360p. Refreshed to `2026.03.17` (YouTube restored to 4K), verified via `scripts/platform-health-check.sh`. Released to users. Open follow-ups: `find_ytdlp()` prefers system yt-dlp over bundled (review); Facebook extraction degraded on stable yt-dlp; runtime yt-dlp self-update (backlog) would decouple extractor freshness from app releases.
